...
// --- [辅助] 自动升级数据库：增加视频封面字段 ---
$check_cover = $pdo->query("SHOW COLUMNS FROM photos LIKE 'video_cover'");
if ($check_cover->rowCount() == 0) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'upload_photo') {
        ...
        if (isset($_FILES['cover_file']) && $_FILES['cover_file']['error'] === UPLOAD_ERR_OK) {
            $tmp_c = $_FILES['cover_file']['tmp_name'];
            $name_c = $_FILES['cover_file']['name'];
            $ext_c = strtolower(pathinfo($name_c, PATHINFO_EXTENSION));
            
            if (in_array($ext_c, ['jpg', 'jpeg', 'png', 'gif', 'webp'])) {
                $newCName = 'cover_' . date('Ymd_His_') . uniqid() . '.' . $ext_c;
                $final_c_url = '';

                if ($cosEnabled == '1') {
                    require_once '../includes/cos_helper.php';
                    $cosPath = 'uploads/' . date('Ym') . '/' . $newCName;
                    $cosUrl = uploadToCOS($tmp_c, $cosPath);
                    if ($cosUrl) $final_c_url = $cosUrl;
                }

                if (empty($final_c_url)) {
                    $uploadDirRel = '../assets/uploads/'; 
                    $uploadDirWeb = '/assets/uploads/';   
                    if (!is_dir($uploadDirRel)) @mkdir($uploadDirRel, 0755, true);
                    if (move_uploaded_file($tmp_c, $uploadDirRel . $newCName)) {
                        $final_c_url = $uploadDirWeb . $newCName;
                    }
                }

                if (!empty($final_c_url)) {
                    $cover_url = $final_c_url; // 如果本地上传了，则覆盖网络链接
                }
            }
        }
        ...
        if (!empty($_POST['network_url'])) {
            ...
            foreach ($urls as $url) {
                ...
                $stmt = $pdo->prepare("INSERT INTO photos (album_id, title, device, image_url, is_featured, video_cover) VALUES (?, ?, ?, ?, ?, ?)");
                $stmt->execute([$album_id, $base_title, $device, $url, $is_featured, $cover_url]);
                $success_count++;
            }
        }
            ...
            for ($i = 0; $i < $file_count; $i++) {
                if ($files['error'][$i] === UPLOAD_ERR_OK) {
                    ...
                    if (in_array($ext, ['jpg', 'jpeg', 'png', 'gif', 'webp', 'mp4', 'webm', 'mov'])) {
                        ...
                        if (!empty($final_url)) {
                            $stmt = $pdo->prepare("INSERT INTO photos (album_id, title, device, image_url, is_featured, video_cover) VALUES (?, ?, ?, ?, ?, ?)");
                            $stmt->execute([$album_id, $base_title, $device, $final_url, $is_featured, $cover_url]);
                            $success_count++;
                        }
                    }
                }
                ...
                <div class="pc-img-box">
                    ...
                    <?php if($is_video): ?>
                        <video src="<?= htmlspecialchars($p['image_url']) ?>" 
                               <?= !empty($p['video_cover']) ? 'poster="'.htmlspecialchars($p['video_cover']).'"' : '' ?>
    ...
    <div class="modal-content">
        <form id="uploadForm" method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-header">
                <h3 class="modal-title">批量上传照片/视频</h3>
            ...
            <div class="modal-body">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group" style="flex-grow:1;">
                        ...
                        <input type="text" name="title" class="form-control" placeholder="应用于所有选中的文件">
                    </div>
                </div>
                ...
                <div class="form-row">
                    ...
                    <div class="form-group" style="flex-grow:1;">
                        <label class="form-label">视频封面 URL (选填)</label>
                        <input type="text" name="cover_url" class="form-control" placeholder="输入封面图直链">
                    </div>
                </div>

                <div class="form-group">
                    <label class="form-label">本地视频封面图 (选填，仅对视频有效)</label>
                    <input type="file" name="cover_file" class="form-control" accept="image/*" style="padding: 8px;">
                    <span style="font-size:12px; color:#94a3b8; margin-top:4px;">提示：如同时填写了URL和上传了本地文件，将优先使用本地上传的封面。</span>
                </div>
            </div>
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary"><i class="fas fa-paper-plane"></i> 开始发布</button>
            </div>
        </form>
    </div>
</div>
